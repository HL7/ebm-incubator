...
This repository is the source for the EBM Incubator implementation guide, housing the PublicationRecord resource.

The initial intent was to include the PublicationRecord Additional resource in the existing EBM-on-FHIR implementation guide. However, the FHIR Shorthand tooling needs to be updated before it will support profiling of additional resources defined within the same implementation guide. As a result, this implementation guide was created to house the PublicationRecord resource, and the existing EBM-on-FHIR implementation guide will then take a dependency on this IG. At some point in the future once the FHIR Shorthand tooling supports profiling of resources defined in the same implementation guide, this IG may be collapsed back in to the EBM-on-FHIR IG.
